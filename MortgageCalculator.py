...
def calc_epp(P, R, N, n):
	...
	"""
	monthly_payment = round(((P / N) + P * (1 - n / N) * R),2);
#	monthly_payment = 0 ## TODO：基于等额本金公式计算，注意保留 2 位小数
...
#	monthly_payment = 0 ## TODO：基于等额本息公式计算，注意保留 2 位小数
...
def calc_monthly_payment(P, R, N, EPP):
	...
	if EPP:
		## 使用等额本金公式
		for n in range(N):
#			monthly_payment = 0 ## TODO：调用函数 calc_epp 计算
			
			print("第 %i 月，需要还款 %.2f", n + 1, calc_epp(P, R, N, n) ) ## TODO：补充信息中的月份和金额
	else:
		## 使用等额本息公式
		for n in range(N):
#			monthly_payment = 0 ## TODO：调用函数 calc_elp 计算
			
			print("第 %i 月，需要还款 %.2f" , n + 1, calc_elp(P, R, N) ) ## TODO：补充信息中的月份和金额
